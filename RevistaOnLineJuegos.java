...
public class RevistaOnLineJuegos 
{
    ...
    public void add(Juego juego) {
        if(estaCompleta()){
            System.out.println("Error, no se puede añadir mas juegos, tamaño limite alcanzado");
        }else if(existeJuego(juego.getTitulo()) >= 0){
            System.out.println("Error, el juego" + juego.getTitulo() + "ya está disponible");
        }else{
            int posicion = 0;
    ...
    public String toString() {
        ...
        sb.append("\nLos mejores juegos en nuestra revista " + " (" + total + " juegos)\n");
        for(int i = 0; i < total; i++){
            sb.append("\n" + juegos[i].toString() + "\n--------------------");
        }
        return sb.toString();
    ...
    public String[] valoracionMayorQue(double valoracion){
        String[] valorados = new String[total];
        int pos = 0;
        for(int i = 0; i < total; i++){
            if(juegos[i].getValoracionMedia() > valoracion){
                valorados[pos] = juegos[i].getTitulo();
                pos++;
            }
        }
        return Arrays.copyOf(valorados, pos);
    }
